...
def parse_det_bottom(Y, C, score=0.1):
    ...
    if len(y_c) > 0:
        for i in range(len(y_c)):
            ...
            y_g_c = y_c[i] + 0.1*h
            h_g = 1.25*h
            for j in range(len(RPN_ANCHOR_RATIOS)):
                w_g = h_g / RPN_ANCHOR_RATIOS[j]
                x1, y1 = max(0, x_g_c[i] * 4 + 2 - w_g / 2), max(0, y_g_c[i] * 4 + 2-h_g)
                boxs.append([x1, y1, min(x1 + w_g, C.size_test[1]), min(y1 + h_g, C.size_test[0]), s])
        boxs = np.asarray(boxs, dtype=np.float32)
        keep = nms(boxs, 0.5, usegpu=False, gpu_id=0)
        boxs = boxs[keep, :]
    return boxs
...
def soft_bbox_vote(det,thre=0.5,score=0.05):
    if det.shape[0] <= 1:
        return det
