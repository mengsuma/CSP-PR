...
C = config.Config()
C.offset = True
cache_path = 'data/cache/cvsm/val_500'
...
if C.offset:
    w_path = 'output/valmodels/cvsm/%s/off' % (C.scale)
    out_path = 'output/valresults/cvsm/%s/off' % (C.scale)
else:
    w_path = 'output/valmodels/cvsm/%s/nooff' % (C.scale)
    out_path = 'output/valresults/cvsm/%s/nooff' % (C.scale)
